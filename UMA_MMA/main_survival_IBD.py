...
from utils.utils import (seed_torch, array2list, merge_dict, read_splits, 
                         parse_model_name, get_current_time, extract_patching_info,save_result_my)

from .trainer import train
from wsi_datasets.wsi_survival_IBD import WSISurvivalDataset
# pytorch imports
import torch
...
def main(args):
    ...
    if args.diff_step > 0:
        train_kwargs['MILemb_path'] = os.path.join(args.split_dir, 'MILembeddings', f'MILemb_train_{args.diff_step}.pkl')
    ...
    if args.diff_step > 0:
        val_kwargs['MILemb_path'] = os.path.join(args.split_dir, 'MILembeddings', f'MILemb_test_{args.diff_step}.pkl')
    
    if '/BRCA/' in args.split_dir:
        text_emb_path = 'path/to/your/text/embeddings/directory'
        
    if args.emb_model_type in ['mocat', 'mcat', 'coattn', 'coattn_mot'] or args.only_MILemb:    
        print('****************use text_emb_path in dataset!!!********************')
        print('text_emb_path:', text_emb_path)
        train_kwargs['text_emb_path'] = text_emb_path
        val_kwargs['text_emb_path'] = text_emb_path
    ...
    fold_results, fold_dumps, result_my = train(dataset_splits, args, mode='survival')
...
#zrf add args
parser.add_argument('--diff_step', type=int, default=30000)
parser.add_argument('--use_MILemb_proto', action='store_true', default=False)
parser.add_argument('--use_MILemb_liner', action='store_true', default=False)
parser.add_argument('--only_MILemb', action='store_true', default=False)
parser.add_argument('--resultmy_path', type=str, default=None)
...
if __name__ == "__main__":
    if sum([args.use_MILemb_proto, args.use_MILemb_liner, args.only_MILemb]) > 1:
        raise ValueError("Cannot use both MILemb_linear and MILemb_proto")
    args.dump = True if args.dump_path is not None else False
    ...
    #new zrf
    if args.wandb_log:
        wandb.init(project='PANTHER', name=args.tags[0],config=vars(args))   
    
    #### train ####
    results,result_my = main(args)
    if args.dump_path is not None:
        save_path=os.path.join(args.dump_path, args.emb_model_type)
        os.makedirs(save_path, exist_ok=True)
        save_path = os.path.join(save_path, f'{args.diff_step}_{args.split_k}.pkl')
        save_pkl(save_path, result_my['dumps_list'])
    
    result_my.pop('dumps_list')
        
    if args.resultmy_path is not None:
        os.makedirs(args.resultmy_path, exist_ok=True)
        
        save_path = os.path.join(args.resultmy_path, f'{args.emb_model_type}.json')
    
        save_result_my({f'diff_step_{args.diff_step}_split_k_{args.split_k}':result_my},save_path)
        
        if args.split_k==4:
            save_result_my({'setting':vars(args)},save_path)
