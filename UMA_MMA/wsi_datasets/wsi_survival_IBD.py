...
class WSISurvivalDataset(Dataset):
    ...
    def __init__(self,
        ...
        #diff_emb
        MILemb_path=kwargs.get('MILemb_path',None)
        print(MILemb_path)
        if MILemb_path is not None:
            self.MILemb=load_pkl(MILemb_path)
            
        self.use_MILemb_proto ,self.use_MILemb_liner ,self.only_MILemb,self.only_MILemb_coattn =False,False,False,False
    ...
    def get_MILemb(self,idx):
        slide_id = self.data_df.loc[self.get_sample_id(idx), 'slide_id']
        if isinstance(slide_id,str):
            slide_id = [slide_id]
        
        emb = []
        for s_id in slide_id:
            if s_id in self.MILemb.keys():
                emb.append(self.MILemb[s_id])
            else:   
                raise ValueError(f'{s_id} not in MILemb')
        
        return torch.mean(torch.stack(emb),dim=0)
    
    def tokenize_MILemb(self):   #NOTE mil emb 修改源头
        for k,v in self.MILemb.items():
            c=v.shape[0]//2   #c is the number of prototypes
            dim = v.shape[1]  #dim is the dimension of the prototype
            prob=torch.ones((c,1))/c
            self.MILemb[k] = torch.concat([prob,v[:c],v[c:]],dim=1)
        
        return c ,dim 
    
    def get_textemb(self,idx):
        slide_id = self.data_df.loc[self.get_sample_id(idx), 'case_id']
        if isinstance(slide_id,str):
            slide_id = [slide_id]
        ...
        for s_id in slide_id:
            try:
                emb.append(torch.load(j_(self.text_emb_path,s_id+'.pt'),weights_only=False))
            except:   
                raise ValueError(f'{s_id} not in MILemb')
    ...
    def __getitem__from_emb__(self, idx):
        if self.only_MILemb:
            MILemb = self.get_MILemb(idx)
            X_gs = self.X[idx]
            img = torch.cat([MILemb[...,1:769],MILemb[...,769:],X_gs],dim=-1)
        ...
        if self.text_emb_path is not None:
            text_emb = self.get_textemb(idx)
    ...
    def __getitem__(self, idx):
        ...
        for feat_path in feat_paths:
            ...
            if len(features.shape) > 2:
                ...
                features.shape[0] == 1, f'{features.shape} is not compatible! It has to be (1, numOffeats, feat_dim) or (numOffeats, feat_dim)'
